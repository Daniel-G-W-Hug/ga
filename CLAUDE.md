...
| `ga/ga_usr_fd.hpp` | the algebra-independent finite-difference substrate the calculus headers build on: `fd_scheme`, `central_scheme` / `compact_scheme` (explicit vs implicit Padé — a compact scheme couples neighbouring derivative values, so it needs a grid and cannot be evaluated at a point), `scheme_at`, `fd_derivative`, `fd_step`. Uniform and non-uniform node sets; weights are generated in NORMALIZED coordinates `(x-x0)/h` and rescaled, since `stencil_t`'s order detection presumes nodes of order unity |
