...
void bind_kin_state2dp(nb::module_& m)
{
    nb::class_<kin_state2dp>(m, "kin_state2dp")
        ...
        .def(
            "__init__",
            [](kin_state2dp* self, vec2dp const& vel, vec2dp const& acc, double omega,
               double alpha) { new (self) kin_state2dp(vel, acc, omega, alpha); },
            nb::arg("vel"), nb::arg("acc"), nb::arg("omega"), nb::arg("alpha"))
        .def_rw("vel", &kin_state2dp::vel)
        .def_rw("acc", &kin_state2dp::acc)
        .def_rw("omega", &kin_state2dp::omega)
        .def_rw("alpha", &kin_state2dp::alpha)
        .def("__repr__", [](const kin_state2dp& v) { return fmt::format("{}", v); })
        .def("__str__", [](const kin_state2dp& v) { return fmt::format("{}", v); })
        .def(
            "__format__",
            [](const kin_state2dp& v, std::string_view spec) {
                try {
            ...
            nb::arg("format_spec"))
        .def("__eq__", [](const kin_state2dp& a, const kin_state2dp& b) {
