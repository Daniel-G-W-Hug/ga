...
template <typename T> struct MVec4_t<T, mvec2dp_u_tag> : public MVec4_t<T, default_tag> {
    ...
    MVec4_t(MVec4_t<U, mvec2dp_u_tag> const& v) : MVec4_t(v.c0, v.c1, v.c2, v.c3)
    {
    }

    // assign a vector part exclusively (other grades = 0)
    MVec4_t(Vec2dp<T> v) : MVec4_t(v.x, v.y, v.z, 0.0) {}

    // assign a scalar part exclusively (other grades = 0)
    MVec4_t(PScalar2dp<T> ps) : MVec4_t(0.0, 0.0, 0.0, T(ps)) {}
