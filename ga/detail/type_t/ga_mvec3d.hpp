#pragma once

// Copyright 2024-2026, Daniel Hug. All rights reserved.
// Licensed under the terms specified in LICENSE.txt file.

#include "ga_type3d.hpp" // type aliases for scalars, vector, pseudoscalar, etc.

#include "ga_mvec3d_e.hpp" // even-grade multivector
#include "ga_mvec3d_u.hpp" // uneven- or odd-grade multivector


namespace hd::ga {

/////////////////////////////////////////////////////////////////////////////////////////
// use MVec8_t including its ctors and add specific ctors for MVec8_t<T, Tag>
// by using partial template specialization for the Tag=mvec3d_tag
/////////////////////////////////////////////////////////////////////////////////////////

template <typename T>
    requires(numeric_type<T>)
struct MVec8_t<T, mvec3d_tag> : public MVec8_t<T, default_tag> {

    using MVec8_t<T, default_tag>::MVec8_t; // inherit base class ctors

    // assign a scalar part exclusively (other grades = 0)
    constexpr MVec8_t(Scalar3d<T> s) :
        MVec8_t(T(s), T(0.0), T(0.0), T(0.0), T(0.0), T(0.0), T(0.0), T(0.0))
    {
    }

    // assign a vector part exclusively (other grades = 0)
    constexpr MVec8_t(Vec3d<T> const& v) :
        MVec8_t(T(0.0), v.x, v.y, v.z, T(0.0), T(0.0), T(0.0), T(0.0))
    {
    }

    // assign a bivector part exclusively (other grades = 0)
    constexpr MVec8_t(BiVec3d<T> const& B) :
        MVec8_t(T(0.0), T(0.0), T(0.0), T(0.0), B.x, B.y, B.z, T(0.0))
    {
    }

    // assign a pseudoscalar part exclusively (other grades = 0)
    constexpr MVec8_t(PScalar3d<T> ps) :
        MVec8_t(T(0.0), T(0.0), T(0.0), T(0.0), T(0.0), T(0.0), T(0.0), T(ps))
    {
    }

    // assign a scalar and a pseudoscalar part exclusively (other grades = 0)
    constexpr MVec8_t(Scalar3d<T> s, PScalar3d<T> ps) :
        MVec8_t(T(s), T(0.0), T(0.0), T(0.0), T(0.0), T(0.0), T(0.0), T(ps))
    {
    }

    // assign a geometric product resulting from a product of two vectors
    // via dot(v1,v2) and wdg(v1,v2) or via dot(v1,v2) and cmt(v1,v2) directly
    // (other grades = 0)
    constexpr MVec8_t(Scalar3d<T> s, BiVec3d<T> const& B) :
        MVec8_t(T(s), T(0.0), T(0.0), T(0.0), B.x, B.y, B.z, T(0.0))
    {
    }

    // assign from a quaternion, i.e. from the even subalgebra
    constexpr MVec8_t(MVec3d_E<T> const& M) :
        MVec8_t(M.c0, T(0.0), T(0.0), T(0.0), M.c1, M.c2, M.c3, T(0.0))
    {
    }

    // assign a geometric product resulting from a product of a vector and a bivector
    constexpr MVec8_t(Vec3d<T> const& v, PScalar3d<T> ps) :
        MVec8_t(T(0.0), v.x, v.y, v.z, T(0.0), T(0.0), T(0.0), T(ps))
    {
    }

    // assign from the odd subalgebra
    constexpr MVec8_t(MVec3d_U<T> const& M) :
        MVec8_t(T(0.0), M.c0, M.c1, M.c2, T(0.0), T(0.0), T(0.0), M.c3)
    {
    }

    // assign a full multivector
    constexpr MVec8_t(Scalar3d<T> s, Vec3d<T> const& v, BiVec3d<T> const& B,
                      PScalar3d<T> ps) :
        MVec8_t(T(s), v.x, v.y, v.z, B.x, B.y, B.z, T(ps))
    {
    }

    // Override compound assignment operators to return correct derived type
    // This ensures GCC+doctest can properly deduce the tag type without needing cross-tag
    // comparisons
    template <typename U>
        requires(numeric_type<U>)
    MVec8_t& operator+=(MVec8_t<U, mvec3d_tag> const& v) noexcept
    {
        this->c0 += v.c0;
        this->c1 += v.c1;
        this->c2 += v.c2;
        this->c3 += v.c3;
        this->c4 += v.c4;
        this->c5 += v.c5;
        this->c6 += v.c6;
        this->c7 += v.c7;
        return *this;
    }

    template <typename U>
        requires(numeric_type<U>)
    MVec8_t& operator-=(MVec8_t<U, mvec3d_tag> const& v) noexcept
    {
        this->c0 -= v.c0;
        this->c1 -= v.c1;
        this->c2 -= v.c2;
        this->c3 -= v.c3;
        this->c4 -= v.c4;
        this->c5 -= v.c5;
        this->c6 -= v.c6;
        this->c7 -= v.c7;
        return *this;
    }

    template <typename U>
        requires(numeric_type<U>)
    MVec8_t& operator*=(U s) noexcept
    {
        this->c0 *= s;
        this->c1 *= s;
        this->c2 *= s;
        this->c3 *= s;
        this->c4 *= s;
        this->c5 *= s;
        this->c6 *= s;
        this->c7 *= s;
        return *this;
    }

    template <typename U>
        requires(numeric_type<U>)
    MVec8_t& operator/=(U s) noexcept(!detail::extended_testing_enabled())
    {
        detail::check_division_by_zero<T, U>(s, "multivector division 8 comp.");
        this->c0 /= s;
        this->c1 /= s;
        this->c2 /= s;
        this->c3 /= s;
        this->c4 /= s;
        this->c5 /= s;
        this->c6 /= s;
        this->c7 /= s;
        return *this;
    }
};

////////////////////////////////////////////////////////////////////////////////
// define grade operations for partial specialization MVec8_t<T, mvec3d_tag>
////////////////////////////////////////////////////////////////////////////////

// returning various grades of a 3d multivector
//
// grade 0: gr0() - scalar
// grade 1: gr1() - vector
// grade 2: gr2() - bivector
// grade 3: gr3() - trivector (= pseudoscalar in 3d)

template <typename T>
    requires(numeric_type<T>)
constexpr Scalar3d<T> gr0(MVec3d<T> const& M)
{
    return Scalar3d<T>(M.c0);
}

template <typename T>
    requires(numeric_type<T>)
constexpr Vec3d<T> gr1(MVec3d<T> const& M)
{
    return Vec3d<T>(M.c1, M.c2, M.c3);
}

template <typename T>
    requires(numeric_type<T>)
constexpr BiVec3d<T> gr2(MVec3d<T> const& M)
{
    return BiVec3d<T>(M.c4, M.c5, M.c6);
}

template <typename T>
    requires(numeric_type<T>)
constexpr PScalar3d<T> gr3(MVec3d<T> const& M)
{
    return PScalar3d<T>(M.c7);
}


////////////////////////////////////////////////////////////////////////////////
// addition operations to combine scalars, pseudoscalar and vectors to multivectors
// (only remaining combinations not covered in mvec3d_e.hpp and mvec3d_u.hpp)
////////////////////////////////////////////////////////////////////////////////

// scalar + vector => multivector
// (bivector and pseudoscalar are set to zero)
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator+(Scalar3d<T> s, Vec3d<U> const& v)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(s, v, BiVec3d<ctype>{}, PScalar3d<ctype>{});
}

// vector + scalar  => multivector
// (bivector and pseudoscalar are set to zero)
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator+(Vec3d<T> const& v, Scalar3d<U> s)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(s, v, BiVec3d<ctype>{}, PScalar3d<ctype>{});
}

// scalar + pseudoscalar (=trivector) => even-grade multivector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator+(Scalar3d<T> s, PScalar3d<U> ps)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(s, Vec3d<ctype>{}, BiVec3d<ctype>{}, ps);
}

// pseudoscalar (=trivector) + scalar => even-grade multivector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator+(PScalar3d<T> ps, Scalar3d<U> s)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(s, Vec3d<ctype>{}, BiVec3d<ctype>{}, ps);
}

// vector + bivector => multivector
// (scalar and pseudoscalar are set to zero)
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator+(Vec3d<T> const& v,
                                                     BiVec3d<U> const& B)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(Scalar3d<ctype>{}, v, B, PScalar3d<ctype>{});
}

// bivector + vector  => multivector
// (scalar and pseudoscalar are set to zero)
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator+(BiVec3d<T> const& B,
                                                     Vec3d<U> const& v)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(Scalar3d<ctype>{}, v, B, PScalar3d<ctype>{});
}

// pseudoscalar + bivector => multivector
// (scalar and vector are set to zero)
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator+(PScalar3d<T> ps, BiVec3d<U> const& B)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(Scalar3d<ctype>{}, Vec3d<ctype>{}, B, ps);
}

// bivector + pseudoscalar  => multivector
// (scalar and vector are set to zero)
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator+(BiVec3d<T> const& B, PScalar3d<U> ps)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(Scalar3d<ctype>{}, Vec3d<ctype>{}, B, ps);
}

// multivector + scalar
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator+(MVec3d<T> const& M, Scalar3d<U> s)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(gr0(M) + s, gr1(M), gr2(M), gr3(M));
}

// multivector + vector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator+(MVec3d<T> const& M,
                                                     Vec3d<U> const& v)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(gr0(M), gr1(M) + v, gr2(M), gr3(M));
}

// multivector + bivector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator+(MVec3d<T> const& M,
                                                     BiVec3d<U> const& B)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(gr0(M), gr1(M), gr2(M) + B, gr3(M));
}

// multivector + pseudoscalar
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator+(MVec3d<T> const& M, PScalar3d<U> ps)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(gr0(M), gr1(M), gr2(M), gr3(M) + ps);
}

////////////////////////////////////////////////////////////////////////////////
// Addition operations completing the type matrix: the remaining pairs whose grade
// union fits no subalgebra, hence returning the full multivector
////////////////////////////////////////////////////////////////////////////////

// scalar + multivector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator+(Scalar3d<T> s, MVec3d<U> const& M)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(s + gr0(M), gr1(M), gr2(M), gr3(M));
}

// vector + multivector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator+(Vec3d<T> const& v,
                                                     MVec3d<U> const& M)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(gr0(M), v + gr1(M), gr2(M), gr3(M));
}

// bivector + multivector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator+(BiVec3d<T> const& B,
                                                     MVec3d<U> const& M)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(gr0(M), gr1(M), B + gr2(M), gr3(M));
}

// pseudoscalar + multivector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator+(PScalar3d<T> ps, MVec3d<U> const& M)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(gr0(M), gr1(M), gr2(M), ps + gr3(M));
}

// even grade multivector + multivector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator+(MVec3d_E<T> const& E,
                                                     MVec3d<U> const& M)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(gr0(E) + gr0(M), gr1(M), gr2(E) + gr2(M), gr3(M));
}

// odd grade multivector + multivector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator+(MVec3d_U<T> const& O,
                                                     MVec3d<U> const& M)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(gr0(M), gr1(O) + gr1(M), gr2(M), gr3(O) + gr3(M));
}

// multivector + even grade multivector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator+(MVec3d<T> const& M,
                                                     MVec3d_E<U> const& E)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(gr0(M) + gr0(E), gr1(M), gr2(M) + gr2(E), gr3(M));
}

// multivector + odd grade multivector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator+(MVec3d<T> const& M,
                                                     MVec3d_U<U> const& O)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(gr0(M), gr1(M) + gr1(O), gr2(M), gr3(M) + gr3(O));
}

// even grade multivector + odd grade multivector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator+(MVec3d_E<T> const& E,
                                                     MVec3d_U<U> const& O)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(gr0(E), gr1(O), gr2(E), gr3(O));
}

// odd grade multivector + even grade multivector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator+(MVec3d_U<T> const& O,
                                                     MVec3d_E<U> const& E)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(gr0(E), gr1(O), gr2(E), gr3(O));
}

// vector + even grade multivector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator+(Vec3d<T> const& v,
                                                     MVec3d_E<U> const& E)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(gr0(E), v, gr2(E), PScalar3d<ctype>{});
}

// even grade multivector + vector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator+(MVec3d_E<T> const& E,
                                                     Vec3d<U> const& v)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(gr0(E), v, gr2(E), PScalar3d<ctype>{});
}

// pseudoscalar + even grade multivector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator+(PScalar3d<T> ps,
                                                     MVec3d_E<U> const& E)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(gr0(E), Vec3d<ctype>{}, gr2(E), ps);
}

// even grade multivector + pseudoscalar
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator+(MVec3d_E<T> const& E,
                                                     PScalar3d<U> ps)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(gr0(E), Vec3d<ctype>{}, gr2(E), ps);
}

// scalar + odd grade multivector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator+(Scalar3d<T> s, MVec3d_U<U> const& O)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(s, gr1(O), BiVec3d<ctype>{}, gr3(O));
}

// odd grade multivector + scalar
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator+(MVec3d_U<T> const& O, Scalar3d<U> s)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(s, gr1(O), BiVec3d<ctype>{}, gr3(O));
}

// bivector + odd grade multivector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator+(BiVec3d<T> const& B,
                                                     MVec3d_U<U> const& O)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(Scalar3d<ctype>{}, gr1(O), B, gr3(O));
}

// odd grade multivector + bivector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator+(MVec3d_U<T> const& O,
                                                     BiVec3d<U> const& B)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(Scalar3d<ctype>{}, gr1(O), B, gr3(O));
}


////////////////////////////////////////////////////////////////////////////////
// subtraction operations to combine scalars, pseudoscalar and vectors to multivectors
// (only remaining combinations not covered in mvec3d_e.hpp and mvec3d_u.hpp)
////////////////////////////////////////////////////////////////////////////////

// scalar - vector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator-(Scalar3d<T> s, Vec3d<U> const& v)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(s, -v, BiVec3d<ctype>{}, PScalar3d<ctype>{});
}

// vector - scalar
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator-(Vec3d<T> const& v, Scalar3d<U> s)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(-s, v, BiVec3d<ctype>{}, PScalar3d<ctype>{});
}

// scalar - pseudoscalar
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator-(Scalar3d<T> s, PScalar3d<U> ps)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(s, Vec3d<ctype>{}, BiVec3d<ctype>{}, -ps);
}

// pseudoscalar - scalar
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator-(PScalar3d<T> ps, Scalar3d<U> s)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(-s, Vec3d<ctype>{}, BiVec3d<ctype>{}, ps);
}

// vector - bivector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator-(Vec3d<T> const& v,
                                                     BiVec3d<U> const& B)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(Scalar3d<ctype>{}, v, -B, PScalar3d<ctype>{});
}

// bivector - vector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator-(BiVec3d<T> const& B,
                                                     Vec3d<U> const& v)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(Scalar3d<ctype>{}, -v, B, PScalar3d<ctype>{});
}

// pseudoscalar - bivector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator-(PScalar3d<T> ps, BiVec3d<U> const& B)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(Scalar3d<ctype>{}, Vec3d<ctype>{}, -B, ps);
}

// bivector - pseudoscalar
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator-(BiVec3d<T> const& B, PScalar3d<U> ps)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(Scalar3d<ctype>{}, Vec3d<ctype>{}, B, -ps);
}

// multivector - scalar
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator-(MVec3d<T> const& M, Scalar3d<U> s)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(gr0(M) - s, gr1(M), gr2(M), gr3(M));
}

// multivector - vector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator-(MVec3d<T> const& M,
                                                     Vec3d<U> const& v)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(gr0(M), gr1(M) - v, gr2(M), gr3(M));
}

// multivector - bivector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator-(MVec3d<T> const& M,
                                                     BiVec3d<U> const& B)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(gr0(M), gr1(M), gr2(M) - B, gr3(M));
}

// multivector - pseudoscalar
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator-(MVec3d<T> const& M, PScalar3d<U> ps)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(gr0(M), gr1(M), gr2(M), gr3(M) - ps);
}

////////////////////////////////////////////////////////////////////////////////
// Subtraction operations completing the type matrix: the remaining pairs whose grade
// union fits no subalgebra, hence returning the full multivector
////////////////////////////////////////////////////////////////////////////////

// scalar - multivector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator-(Scalar3d<T> s, MVec3d<U> const& M)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(s - gr0(M), -gr1(M), -gr2(M), -gr3(M));
}

// vector - multivector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator-(Vec3d<T> const& v,
                                                     MVec3d<U> const& M)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(-gr0(M), v - gr1(M), -gr2(M), -gr3(M));
}

// bivector - multivector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator-(BiVec3d<T> const& B,
                                                     MVec3d<U> const& M)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(-gr0(M), -gr1(M), B - gr2(M), -gr3(M));
}

// pseudoscalar - multivector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator-(PScalar3d<T> ps, MVec3d<U> const& M)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(-gr0(M), -gr1(M), -gr2(M), ps - gr3(M));
}

// even grade multivector - multivector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator-(MVec3d_E<T> const& E,
                                                     MVec3d<U> const& M)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(gr0(E) - gr0(M), -gr1(M), gr2(E) - gr2(M), -gr3(M));
}

// odd grade multivector - multivector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator-(MVec3d_U<T> const& O,
                                                     MVec3d<U> const& M)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(-gr0(M), gr1(O) - gr1(M), -gr2(M), gr3(O) - gr3(M));
}

// multivector - even grade multivector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator-(MVec3d<T> const& M,
                                                     MVec3d_E<U> const& E)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(gr0(M) - gr0(E), gr1(M), gr2(M) - gr2(E), gr3(M));
}

// multivector - odd grade multivector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator-(MVec3d<T> const& M,
                                                     MVec3d_U<U> const& O)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(gr0(M), gr1(M) - gr1(O), gr2(M), gr3(M) - gr3(O));
}

// even grade multivector - odd grade multivector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator-(MVec3d_E<T> const& E,
                                                     MVec3d_U<U> const& O)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(gr0(E), -gr1(O), gr2(E), -gr3(O));
}

// odd grade multivector - even grade multivector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator-(MVec3d_U<T> const& O,
                                                     MVec3d_E<U> const& E)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(-gr0(E), gr1(O), -gr2(E), gr3(O));
}

// vector - even grade multivector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator-(Vec3d<T> const& v,
                                                     MVec3d_E<U> const& E)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(-gr0(E), v, -gr2(E), PScalar3d<ctype>{});
}

// even grade multivector - vector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator-(MVec3d_E<T> const& E,
                                                     Vec3d<U> const& v)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(gr0(E), -v, gr2(E), PScalar3d<ctype>{});
}

// pseudoscalar - even grade multivector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator-(PScalar3d<T> ps,
                                                     MVec3d_E<U> const& E)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(-gr0(E), Vec3d<ctype>{}, -gr2(E), ps);
}

// even grade multivector - pseudoscalar
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator-(MVec3d_E<T> const& E,
                                                     PScalar3d<U> ps)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(gr0(E), Vec3d<ctype>{}, gr2(E), -ps);
}

// scalar - odd grade multivector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator-(Scalar3d<T> s, MVec3d_U<U> const& O)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(s, -gr1(O), BiVec3d<ctype>{}, -gr3(O));
}

// odd grade multivector - scalar
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator-(MVec3d_U<T> const& O, Scalar3d<U> s)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(-s, gr1(O), BiVec3d<ctype>{}, gr3(O));
}

// bivector - odd grade multivector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator-(BiVec3d<T> const& B,
                                                     MVec3d_U<U> const& O)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(Scalar3d<ctype>{}, -gr1(O), B, -gr3(O));
}

// odd grade multivector - bivector
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
constexpr MVec3d<std::common_type_t<T, U>> operator-(MVec3d_U<T> const& O,
                                                     BiVec3d<U> const& B)
{
    using ctype = std::common_type_t<T, U>;
    return MVec3d<ctype>(Scalar3d<ctype>{}, gr1(O), -B, gr3(O));
}

} // namespace hd::ga