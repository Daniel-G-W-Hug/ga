...
struct MVec8_t<T, mvec2dc_e_tag> : public MVec8_t<T, default_tag> {
    ...
    constexpr MVec8_t(Scalar2dc<T> s, BiVec2dc<T> const& B) :
    ...
    constexpr MVec8_t(BiVec2dc<T> const& B, PScalar2dc<T> ps) :
    ...
    constexpr MVec8_t(Scalar2dc<T> s, PScalar2dc<T> ps) :
    ...
    constexpr MVec8_t(Scalar2dc<T> s, BiVec2dc<T> const& B, PScalar2dc<T> ps) :
    ...
    MVec8_t& operator+=(MVec8_t<U, mvec2dc_e_tag> const& v) noexcept
    {
        this->c0 += v.c0;
        this->c1 += v.c1;
        this->c2 += v.c2;
...
////////////////////////////////////////////////////////////////////////////////
// define grade operations for partial specialization MVec8_t<T, mvec2dc_e_tag>
////////////////////////////////////////////////////////////////////////////////

// returning various grades of an even 2dc multivector
//
// grade 0: gr0() - scalar
// grade 2: gr2() - bivector
// grade 3: gr4() - quadvector (= pseudoscalar in 2dc)

template <typename T>
    requires(numeric_type<T>)
constexpr Scalar2dc<T> gr0(MVec2dc_E<T> const& M)
{
...
template <typename T>
    requires(numeric_type<T>)
constexpr BiVec2dc<T> gr2(MVec2dc_E<T> const& M)
{
    return BiVec2dc<T>(M.c1, M.c2, M.c3, M.c4, M.c5, M.c6);
}

template <typename T>
    requires(numeric_type<T>)
constexpr PScalar2dc<T> gr4(MVec2dc_E<T> const& M)
{
    return PScalar2dc<T>(M.c7);
}
...
// scalar + bivector => even-grade multivector (in 2dc imply a zero pseudoscalar)
...
// pseudoscalar + bivector => even-grade multivector (in 2dc imply a zero scalar)
template <typename T, typename U>
    requires(numeric_type<T> && numeric_type<U>)
...
{
    using ctype = std::common_type_t<T, U>;
    return MVec2dc_E<ctype>(T(s) + M.c0, M.c1, M.c2, M.c3, M.c4, M.c5, M.c6, M.c7);
}
...
{
    using ctype = std::common_type_t<T, U>;
    return MVec2dc_E<ctype>(U(s) + M.c0, M.c1, M.c2, M.c3, M.c4, M.c5, M.c6, M.c7);
}
...
{
    using ctype = std::common_type_t<T, U>;
    return MVec2dc_E<ctype>(M.c0, B.vx + M.c1, B.vy + M.c2, B.vz + M.c3, B.mx + M.c4,
...
{
    using ctype = std::common_type_t<T, U>;
    return MVec2dc_E<ctype>(M.c0, M.c1, M.c2, M.c3, M.c4, M.c5, M.c6, T(ps) + M.c7);
}
...
{
    using ctype = std::common_type_t<T, U>;
    return MVec2dc_E<ctype>(M.c0, M.c1, M.c2, M.c3, M.c4, M.c5, M.c6, U(ps) + M.c7);
}
...
{
    using ctype = std::common_type_t<T, U>;
    return MVec2dc_E<ctype>(-M.c0, -M.c1, -M.c2, -M.c3, -M.c4, -M.c5, -M.c6,
                            T(ps) - M.c7);
}
