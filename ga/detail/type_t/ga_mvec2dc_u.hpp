...
struct MVec8_t<T, mvec2dc_u_tag> : public MVec8_t<T, default_tag> {
    ...
    MVec8_t& operator+=(MVec8_t<U, mvec2dc_u_tag> const& v) noexcept
    {
        this->c0 += v.c0;
        this->c1 += v.c1;
        this->c2 += v.c2;
...
////////////////////////////////////////////////////////////////////////////////
// define grade operations for partial specialization MVec8_t<T, mvec2dc_u_tag>
////////////////////////////////////////////////////////////////////////////////

// returning various grades of a 2dc odd multivector
//
// grade 1: gr1() - vector
...
template <typename T>
    requires(numeric_type<T>)
constexpr Vec2dc<T> gr1(MVec2dc_U<T> const& M)
{
    return Vec2dc<T>(M.c0, M.c1, M.c2, M.c3);
}

template <typename T>
    requires(numeric_type<T>)
constexpr TriVec2dc<T> gr3(MVec2dc_U<T> const& M)
{
    return TriVec2dc<T>(M.c4, M.c5, M.c6, M.c7);
}
...
{
    using ctype = std::common_type_t<T, U>;
    return MVec2dc_U<ctype>(M.c0, M.c1, M.c2, M.c3, M.c4 + t.x, M.c5 + t.y, M.c6 + t.z,
                            M.c7 + t.w);
}
...
{
    using ctype = std::common_type_t<T, U>;
    return MVec2dc_U<ctype>(M.c0 + v.x, M.c1 + v.y, M.c2 + v.z, M.c3 + v.w, M.c4, M.c5,
                            M.c6, M.c7);
}
...
{
    using ctype = std::common_type_t<T, U>;
    return MVec2dc_U<ctype>(-M.c0, -M.c1, -M.c2, -M.c3, -M.c4 + t.x, -M.c5 + t.y,
                            -M.c6 + t.z, -M.c7 + t.w);
}
...
{
    using ctype = std::common_type_t<T, U>;
    return MVec2dc_U<ctype>(-M.c0 + v.x, -M.c1 + v.y, -M.c2 + v.z, -M.c3 + v.w, -M.c4,
                            -M.c5, -M.c6, -M.c7);
}
