...
/////////////////////////////////////////////////////////////////////////////////////////
// provides the OPT-IN closed-loop (KKT) layer on top of the pga3dp mechanics:
//
// dynamic_system3dp integrates a spanning TREE. This header adds the loop closures that
// turn the tree into a mechanism, solved as a constrained system. Open-loop users never
// include it.
//
// - constraint3dp         -> the closure kinds: coincidence (pin / ball), distance (rod),
//                            frame (weld) -- each removing its own subspace of relative
//                            motion, i.e. contributing its rows of G
// - loop_constraint3dp    -> one closure: the two frames, their anchors, kind, length,
//                            and whether it is currently active
// - closed_loop_system3dp -> the solver
//
// Building it (the tree-side calls are forwarded to the wrapped dynamic_system3dp):
//
...
class closed_loop_system3dp {
  ...
  public:
    ...
    // The CONSTRAINED analogue of dynamic_system3dp::sync_accelerations(): solve the
    ...
    // q-dot)) for energy-clean drift control. That projection is for ROUND-OFF only: a
    // constraint switched on while the velocities disagree with it is an impact, and
    // impact() (the mass-weighted jump) must be applied before the next step -- see
    // activate_loop_with_impact().
