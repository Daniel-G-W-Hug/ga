...
// - joint_range2dp      -> where a 1-dof joint MAY GO (an interval on q plus the
...
// - joint_drive2dp      -> what its actuator CAN DELIVER (force / rate / rate-change
...
// coordinate q of a 1-dof joint (revolute, prismatic), in the units q carries (rad for
// a hinge, m for a slider). The 3D twin is joint_range3dp: same fields, same neutral
// value, one more joint kind to serve (helical).
//
...
//
// A scalar interval is what a hinge's or a slider's limit IS, and a pseudoscalar adds
...
// resolved in exactly one place instead of travelling through the model. In the plane
// the free joint is the only one whose
// configuration is a MOTOR rather than a scalar, and its restriction is a region of its
// screw subspace -- a box and an angle -- which is its own type, not a wider interval.
//
...
// message at all. Hence the free operator== below, which joint_state2dp's generated
// equality needs and which a member would have cost the binding.
struct joint_range2dp {
...
inline bool operator==(joint_range2dp const& a, joint_range2dp const& b)
{
...
inline bool is_unrestricted(joint_range2dp const& r)
{
...
// geometry says where a joint IS, joint_range2dp where it MAY GO, and this what it CAN
// DO. A model stating the first two and not the third describes a mechanism nobody can
// build. The 3D twin is joint_drive3dp: same fields, same neutral value.
//
// The neutral value is an IDEAL actuator (unbounded force, unbounded speed, unbounded
// acceleration, no reflected inertia), the same kind of neutral value joint_range2dp
...
inline bool operator==(joint_drive2dp const& a, joint_drive2dp const& b)
{
...
struct joint_state2dp {
    ...
    // exactly as it did. Two objects rather than one: see joint_range2dp /
    // joint_drive2dp for what each means and why they are kept apart.
    joint_range2dp range{};
    joint_drive2dp drive{};
...
class dynamic_system2dp : public kinematic_system2dp {
  ...
  public:
    ...
    // go, joint_range2dp) and its actuator CAPABILITY (what the drive can deliver,
    // joint_drive2dp). Both hold their NEUTRAL values -- unrestricted, ideal -- until
    ...
    void set_joint_range(size_t idx, joint_range2dp const& r)
    {
        if (!(r.lo <= r.hi))
            throw std::runtime_error(
                "dynamic_system2dp: a joint range must have lo <= hi.");
        if (r.k_stop < 0.0 || r.c_stop < 0.0)
            throw std::runtime_error("dynamic_system2dp: a joint stop's stiffness and "
        ...
        if (!joint[idx].screws.empty() && !is_unrestricted(r))
            throw std::runtime_error(
                "dynamic_system2dp: a scalar joint range cannot restrict a motor "
        ...
        if (joint[idx].screws.empty() && (joint[idx].phi < r.lo || joint[idx].phi > r.hi))
            throw std::runtime_error(std::string("dynamic_system2dp: joint coordinate ") +
    ...
    void set_joint_drive(size_t idx, joint_drive2dp const& dr)
    {
        if (dr.tau_max < 0.0 || dr.qd_max < 0.0 || dr.qdd_max < 0.0 || dr.armature < 0.0)
            throw std::runtime_error(
                "dynamic_system2dp: an actuator capability must not be negative (the "
