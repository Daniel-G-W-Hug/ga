...
class ground_contact3dp {
  ...
  private:
    ...
    // anchored at its projection onto the ground plane and switched on, and ONE impact
    // map resolves them together. Each event records that shared Lambda (all active
    // rows).
    void touchdown(std::vector<size_t> const& idxs)
    {
        std::vector<vec3dp> at;
        at.reserve(idxs.size());
        for (size_t idx : idxs) {
            contact& c = contacts_[idx];
            vec3dp const landing = unitize(project_onto(contact_point(idx), Pi_));
            if (c.spec.kind == contact_kind3dp::flat) {
