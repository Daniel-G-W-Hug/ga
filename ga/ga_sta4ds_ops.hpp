...
template <typename T>
    requires(numeric_type<T>)
inline std::array<Vec4ds<T>, 4> reciprocal_frame(Vec4ds<T> const& a1, Vec4ds<T> const& a2,
                                                 Vec4ds<T> const& a3, Vec4ds<T> const& a4)
{
    using sta::operator*;
    PScalar4ds<T> const A = wdg(wdg(wdg(a1, a2), a3), a4);
    if (std::abs(T(A)) <= detail::safe_epsilon<T>()) {
        throw std::invalid_argument("reciprocal_frame: degenerate frame");
    }
    PScalar4ds<T> const Ai = inv(A);
    return {wdg(wdg(a2, a3), a4) * Ai, -(wdg(wdg(a1, a3), a4) * Ai),
            wdg(wdg(a1, a2), a4) * Ai, -(wdg(wdg(a1, a2), a3) * Ai)};
}
