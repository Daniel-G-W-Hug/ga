...
class active_frame_trafo : public QObject, public QGraphicsItem {
  ...
  public:
    ...
    active_frame_trafo(Coordsys* cs, w_Coordsys* wcs, frame_trafo_params const& params,
  ...
  private:
    ...
    void drawWorldFrame(QPainter* qp, vec2dp const& O_w, vec2dp const& e1_w,
                        vec2dp const& e2_w);
    void drawTrajectory(QPainter* qp);
    void drawBody(QPainter* qp, vec2dp const& O_w, vec2dp const& e1_w,
                  vec2dp const& e2_w);
    void drawHomeMarker(QPainter* qp); // M0 reference: body position when B_b=0
    void drawGeneratorVectors(QPainter* qp, vec2dp const& O_w, vec2dp const& e1_w,
                              vec2dp const& e2_w);
    ...
    static constexpr double POLE_OFFSET =
        0.20; // pole base offset along e1_b from body origin
