...
- ga_bindgen: scanner / binding generator that parses `ga/` headers with libclang and emits Python bindings (and, in future, audits other wrappers)
- ga_py: Python wrapper around the `ga/` library, generated by ga_bindgen on top of nanobind
...
- ga_view: fmt, qt6
- ga_prdxpr: fmt
- ga_bindgen: Python 3.10+, libclang (provided by a full LLVM install; the pip `libclang` package alone is insufficient on macOS because its resource directory is missing), Python package `libclang`
- ga_py: fmt, Python 3.10+, Python packages `nanobind` (build) and `pytest` (tests)
...
```bash
    ...
    brew install python      // only required for ga_bindgen / ga_py (>= 3.10)

    # Python packages for ga_bindgen + ga_py — install into a venv:
    python3 -m venv .venv && source .venv/bin/activate
    pip install libclang nanobind pytest
```
...
```bash
    ...
    sol2 to be installed from: <https://github.com/ThePhD/sol2>
    LLVM (for libclang) to be installed from: https://releases.llvm.org/
        — only required for ga_bindgen
    winget install --id Python.Python.3.12
        — only required for ga_bindgen / ga_py (any 3.10+ works)

    REM Python packages for ga_bindgen + ga_py — install into a venv:
    python -m venv .venv
