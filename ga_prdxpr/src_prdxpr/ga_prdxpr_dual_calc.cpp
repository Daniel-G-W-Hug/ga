...
static std::string build_dual_expression(std::vector<int> const& coefficients,
                               ...
                               prd_rules const& complement_rules)
{
    ...
    for (size_t i = 0; i < n; ++i) {
        ...
        std::string dual_expr =
            build_dual_expression(row_coeffs, basis, complement_rules);
                                          ...
                                          prd_rules const& complement_rules)
{
    ...
    // For each basis element i: regressive_dual(basis[i]) = Σⱼ Ḡ[i,j] *
    // complement(basis[j])
    for (size_t i = 0; i < n; ++i) {
        std::string const& elem = basis[i];
