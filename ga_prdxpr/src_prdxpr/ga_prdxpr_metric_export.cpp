...
static size_t calculate_max_row_length(
...
static std::string format_matrix_row(
    std::vector<int> const& matrix_data,
    size_t row,
    size_t n,
    ...
    size_t max_row_length)
{
    ...
    size_t padding = max_row_length > current_length ? max_row_length - current_length : 0;
    ...
    std::vector<int> const& matrix_data)
{
    ...
    for (size_t row = 0; row < n; ++row) {
        ...
        oss << format_matrix_row(matrix_data, row, n, basis[row], is_last_row, max_row_length);
    }
    ...
    std::vector<int> const& matrix_data)
{
    ...
    // Header comment
    oss << "// Regressive extended metric for " << algebra_name
        << " (" << n << "×" << n << " matrix, " << total_elements << " elements)\n";
...
void print_metrics_for_algebra(AlgebraConfig const& config)
{
    ...
    std::string extended_code = export_extended_metric_to_code(
        algebra_name, basis, G_data);
    std::cout << extended_code << "\n";

    // For PGA algebras, also export regressive metric
    bool is_pga = (config.metric_signature.back() == 0);  // Last basis vector is degenerate
    ...
    if (is_pga) {
        ...
        auto G_reg_data = calculate_regressive_extended_metric_matrix_full(config, complement_rules);
