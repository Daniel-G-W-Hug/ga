...
\begin{subequations}
    \begin{align}
        ...
        \label{eq:vec2d} \\
        ps_{2d} & = ps \, \bv{12}  = ps \, \pseudoscalar
        \label{eq:pscalar2d}
    \end{align}
\end{subequations}
...
pseudoscalar part $ps \, \bv{12} = ps \, \pseudoscalar$ (basis element is $I_{2d} =
\bv{12}$, which is sometimes written as $\pseudoscalar$ to show its character as
pseudoscalar of this space. $I_{2d}$ is a bivector in $2d$ Euclidean space). \\
...
\begin{subequations}
    \begin{align}
        ...
        \label{eq:bivec3d} \\ 
        ps_{3d} & = ps \, \bv{123}  = ps \, \pseudoscalar
        \label{eq:pscalar3d}
    \end{align}
\end{subequations}
...
and the pseudoscalar part $ps \, \bv{123} = ps \, \pseudoscalar$ (basis element is $I_{3d}
= \bv{123}$, which is sometimes written as $\pseudoscalar$ to show its character as
pseudoscalar of this space. $I_{3d}$ is a trivector in $3d$ Euclidean space). \\
...
\begin{subequations}
    \begin{align}
        ...
        \label{eq:bivec2dp} \\ 
        ps_{2dp} & = ps \, \bv{321}  = ps \, \pseudoscalar
        \label{eq:pscalar2dp}
    \end{align}
\end{subequations}
...
to $0$, because of it's component \bv{3}. Equation (\ref{eq:scalar2dp}) is the scalar part
$s_{2dp}$, equation (\ref{eq:vec2dp}) is the vector part $v_{2dp}$, equation
(\ref{eq:bivec2dp}) is the bivector part $B_{2dp}$, and equation (\ref{eq:pscalar2dp}) is
the pseudoscalar part $ps_{2dp}$. The index $2dp$ is typically omitted when clear from
context. The eight basis elements are $\{\bm{1}, \bv{1}, \bv{2}, \bv{3}, \bv{23}, \bv{31},
\bv{12}, \bv{321}\}$. Using this basis a multivector $M$ of two-dimensional projective
...
(basis element is the scalar \textbf{1}, the vector part $v$ (basis elements \bv{1},
\bv{2} and \bv{3}), the bivector part $b$ (basis elements \bv{23}, \bv{31} and \bv{12})
and the pseudoscalar part $ps \, \bv{321} = ps \, \pseudoscalar$ (basis element is
$I_{2dp} = \bv{321}$, which is sometimes written as $\pseudoscalar$ to show its character
as pseudoscalar of this space. It is a trivector in $2dp$ Euclidean space that squares to
zero). \\
...
\begin{subequations}
    \begin{align}
        ...
        \label{eq:trivec3dp} \\ 
        ps_{3dp} & = ps \, \bv{1234}  = ps \, \pseudoscalar
        \label{eq:pscalar3dp}
    \end{align}
\end{subequations}
Now we have a basis vector~\bv{4} that squares to zero. The bivector has 6 components, and
there is a trivector $t$. The pseudoscalar \bv{1234} is a quadvector that squares to $0$,
because of it's component \bv{4}. Equation (\ref{eq:scalar3dp}) is the scalar part
$s_{3dp}$, equation (\ref{eq:vec3dp}) is the vector part $v_{3dp}$, equation
(\ref{eq:bivec3dp}) is the bivector part $B_{3dp}$, equation (\ref{eq:trivec3dp}) is the
trivector part $t_{3dp}$ and equation (\ref{eq:pscalar3dp}) is the pseudoscalar part
$ps_{3dp}$. The index $3dp$ is typically omitted when clear from context. The sixteen
...
(basis element is the scalar \textbf{1}), the vector part $v$ (basis elements \bv{1},
\bv{2}, \bv{3} and \bv{4}), the bivector part $B$ (basis elements \bv{41}, \bv{42},
\bv{43}, \bv{23}, \bv{31} and \bv{12}), the trivector part $t$ (basis elements \bv{423},
\bv{431}, \bv{412} and \bv{321}) and the pseudoscalar part $ps \, \bv{1234} = ps \,
\pseudoscalar$ (basis element is $I_{3dp} = \bv{1234}$, which is sometimes written as
$\pseudoscalar$ to show its character as pseudoscalar of this space. It is a quadvector in
$3dp$ Euclidean space that squares to zero). \\
...
typically down to a level that reflects the actual need of information to solve the
underlying application problem. \\ 
...
In a two-dimensional algebra, like $ega2d$, the effect of the right dual on a vector $u$
...
\begin{subeqnarray}
    A^{\star} & = &  \revtilde{A} \wedgedot I_n \slabel{eq:hodge_right} \\
    A_{\star} & = & I_n \wedgedot \revtilde{A} \slabel{eq:hodge_left}
\end{subeqnarray}
with $I_n = \bv{1} \wedge \bv{2} \wedge \ldots \wedge \bv{n}$ as the pseudoscalar of the
$n$-dimensional space, and $\revtilde{A}$ is the reversion operation applied to $A$ (e.g.
with $A = \bv{1} \wedge \bv{2}$ the reverse is $\revtilde{A} = \bv{2} \wedge \bv{1}$), and
$\wedgedot$ is the symbol for the geometric product. Following identities link the outer
product and the inner product and hold for operands of same grade: 
\begin{subeqnarray}
...
ToDo: \emph{here an Introducion of the metric and the extended metric is needed (can also
happen in the text below)} \\
...
products, as defined in~(\ref{eq:rwdg_std}) or (\ref{eq:rwdg_alt}). \\
For projective algebras, like $pga$, we use the bulk part for the dualization operation
with the Hodge star (symbolized by a filled star). It is the dualization operation applied
to the bulk part of an object, the bulk dual. The need for a distinction between bulk and
weight is a consequence of the extended degenerate metric, which results in some terms,
i.e. the weight terms, being multiplied by zero in the dualization operation. \\
To be able to dualize the other part of the object as well, there is the dualization
operation specifically applied to the weight part of an object, the weight dual
(symbolized by an unfilled star). \\


The multiplicative identity of the geometric product is the scalar $\bm{1}$:
\begin{equation}
    1 \wedgedot a = a \wedgedot 1 = a
...
$\pseudoscalar = I_n$ of the respective algebra:
\begin{equation}
    \pseudoscalar \veedot a = a \veedot \pseudoscalar = a
...
\begin{eqnarray}
    ...
    \overline{\bv{1}} \veedot \overline{\bv{1}} = \pseudoscalar \nonumber\\
    \overline{\bv{2}} \veedot \overline{\bv{2}} = \pseudoscalar \\
...
\begin{equation}
    A \cdot B = A^{T} \mathds{G} B
    \label{eg:inner_product_metric}
\end{equation}
with $\mathds{G}$ as the extended metric and matrix multiplication on the right hand side.
...
$k=1$ (it has no well-defined name yet). \\
...
(regressive) geometric products in \path{ga_prdxpr}. \\


ToDo: \emph{fill in further basic formulae here as required} \\

TRIALS:
Text with a norm $\nrm{x}$ and an indexed norm as $\bulknrm{u}$ and $\weightnrm{u}$.
$\filledstar$, $\smallstar$ \\
