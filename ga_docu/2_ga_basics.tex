...
(degenerate) metric, which results in some terms, i.e. the weight terms, being multiplied
by zero). The ``left'' operations are typically used on the left-hand side of the
operation and the ``right'' operations on the right-hand side of the product operator. The
only deviation from that pattern occurs for the definition of the regressive products, as
defined in~(\ref{eq:rwdg_std}) or (\ref{eq:rwdg_alt}). \\
