...
($\lcmpl{u}$ or $\rcmpl{u}$) is orthogonal to $u$. The complement is defined as given
above in equations~(\ref{eq:cmpl_def}) for any basis element. It is extended to full
multivectors $A$ and $B$ by linearity as
\begin{subeqnarray}
    % use \label for full label
    % use \slabel for sublabel
...
Based on this it is possible to define so-called regressive products using rules analog to
