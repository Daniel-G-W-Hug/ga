...
as pseudoscalar of this space. It is a trivector in two-dimesional Euclidean projective
...
\bv{412}, \bv{321}) and the pseudoscalar part $ps \, \bv{1234} = ps \, \pseudoscalar$
(basis element is $I_{3dp} = \bv{1234}$, which is sometimes written as $\pseudoscalar$ to
...
The right bulk dual \rightbulkdual{A} is defined as $\rightbulkdual{A} =
\overline{\mathbf{G}A} = \overline{\bulk{A}}$, where $A$ is an arbitrary multivector,
$\mathbf{G}$ the extended metric and $\mathbf{G}A$ a matrix-vector-product between them.
\rightbulkdual{A} is also called the Hodge dual. There is also a corresponding left dual
operation defined as $\leftbulkdual{A} = \underline{\mathbf{G}A} = \underline{\bulk{A}}$.
The Hodge star (symbolized by a filled star $\star$) is used as dualization operator in
both cases. The difference between the hodge dual and the complement is multiplying the
multivector with the extended metric \emph{before} taking the corresponding complement.
However, this does not lead to different results in spaces where the metric is the
identity matrix. It is only relevant for spaces with a more involved metric, especially so
in projective spaces like $pga$ with a non-invertible, i.e. degenerate metric. \\
...
This operation (symbolized by an unfilled star $\smallstar$)uses the regressive metric and
is defined for the right weight dual \rightweightdual{A} as $\rightweightdual{A} =
\overline{\mathbf{\mathbb{G}}A} = \overline{\weight{A}}$ for an arbitrary multivector $A$.
The corresponding left weight dual \leftweightdual{A} is defined as $\leftweightdual{A} =
\underline{\mathbf{\mathbb{G}}A} = \underline{\weight{A}}$.\\
...
\begin{subeqnarray}
    \rightbulkdual{A} & = &  \revtilde{A} \wedgedot I_n \slabel{eq:hodge_right} \\
    \leftbulkdual{A} & = & I_n \wedgedot \revtilde{A} \slabel{eq:hodge_left}
\end{subeqnarray}
with $I_n = \bv{1} \wedge \bv{2} \wedge \ldots \wedge \bv{n}$ as the pseudoscalar of the
