...
object $u$ into the constituting basis elements of the complement of the object $\lcmpl{u}$
or $\rcmpl{u}$. Connecting the object and its complement with the wedge products creates an
...
the space. An implication of the definition is that the complement ($\lcmpl{u}$ or
$\rcmpl{u}$) is orthogonal to $u$. \\
...
the $n$-dimensional space. $\rev{A}$ is the reversion operation, and $\undertilde{A}$
is the regressive reversion operation applied to $A$ (e.g. with $A = \bv{1} \wedge \bv{2}$
the reverse is $\rev{A} = \bv{2} \wedge \bv{1}$), and ``$\wedgedot$'' and
``$\veedot$'' are the symbols for the geometric and regressive geometric products. For an
overview on all bulk and weight duals in $pga3dp$ see table
\ref{tab:pga3dp_bulk_weight_duals} at the end of this section.\\
