...
as resulting bivectors for the case of $PGA2DP$:
\begin{subeqnarray}
    O \wedge f & = &
    \begin{pmatrix}
        0 \\ 0 \\ 1 
...
component of the bivector $F$. For the case of $PGA3DP$ the encoding is similar, but of
...
be retrieved from $F$, either using any known point $P$ on $F$\footnote{Which is in
principle always available via the function $support(F)$. It provides the point on $F$
with the shortest distance to the origin $O$.}, or alternatively by projecting $R$ onto
$F$ and using the perpendicular distance vector $d_\perp$ from $R$ towards the line $F$ in
order to calculate the resulting free torque $M_F(R)$ as
\begin{subeqnarray}
    M_F(R) & = &  (P-R) \wedge f = (P-R) \wedge att(F)
    \slabel{eq:action_line_moment_simple} \\
    M_F(R) & = & d_\perp \wedge f  = (F \vee (R \wedge \rightweightdual{F}) - R) \wedge
    att(F)
...
Please remember:
\begin{itemize}
    \item Objects modelled in projective space can be multiplied by a scalar value without
    changing the geometric meaninig of the object in the modelled Euclidean space, i.e.
    after the projection. Equality is often only defined up to a scalar factor. This is
    defined as congruence. This is also true for $F$. Those factors will be removed during
    unitization, e.g. when the line of action is actually drawn.
    ...
    meaning (see~\cite{Browne_Grassmann-Algebra_Vol1:2012}).
    \item A free torque is modelled by a bivector with force components being zero, i.e.
    the force vector has no direction and zero length.
    \item The torque calculated from either equation (\ref{eq:action_line_moment_simple})
    or (\ref{eq:action_line_moment_projection}) are necessary free torque values as can be
    seen from the formulas: The origin does not occur in the calculation any more due to
    the difference of the two projective points involved.
    \item The sum of forces and their induced torque values can be directly created by the
    sum of their respective line of action, i.e. the bivectors representing those lines.
    The resulting lines of action contain all relevant information incl. the resulting
    vector sum of the forces and the resulting torque values from the summation. There is
    no need to calculate these separately. The result will always be a resulting line of
    action that contains the sum of a bound vector and a free bivector
