...
positions at infinity. In projective geometry the objects must be projected into the
...
in the embedding projective space (a $2d$-object), and so on for higher-grade objects. The
projection reduces the dimension of the object in the embedding space by one. \\
...
line of action then $F$ is the resulting line of action:
\begin{equation}
F = Q \wedge f
...
torque caused by the force either at the origin or at any point in space as shown below.
\\
...
while the second term encodes the resulting free torque relative to the origin caused by
...
components are shown on the right-hand side in square brackets for reference):
\begin{subeqnarray}
    O \wedge f & = &
    \begin{pmatrix}
        0 \\ 0 \\ 1 
...
of three (the corresponding basis bivector compontents are shown on the right-hand side in
square brackets for reference):
\begin{subeqnarray}
    O \wedge f & = &
    \begin{pmatrix}
        0 \\ 0 \\ 0 \\ 1 
...
available via the function $sup(F)$. It provides the point on $F$ with the shortest
...
\begin{itemize}
    ...
    the force vector has no direction and zero length, but the bivector still might
    contain a resulting torque value.
...
within one entity. This is in complete analogy to what we have seen in the last subsection
for force and torque resulting in a force line of action. \\
...
The last missing piece for dynamics is the time derivative of $P_{total}$ expressed as
time rate of change of the inertia. Applying eqn.~(\ref{eq:rcmt_pga_world}) in the body
frame, which is not only valid for any $X$, but for any object moved by a motor $M$, we
get
\begin{equation}
...
\end{eqnarray}
Together with the ansatz in the body system eqn.~(\ref{eq:motor_pga_body_frame}) 
\begin{equation*}
   M(t) = M_0 \veedot exp_\veedot(\frac{1}{2}B_b(t)) 
\end{equation*}
and the rate of change in the body frame according to
eqn.~(\ref{eq:motor_rate_body_frame_pga}) we get the system
\begin{subeqnarray}
    \dot{\Omega}_b &=& I_b^{-1}\left[ F_{b} - rcmt(\Omega_b,I_b[\Omega_b]) \right] \\
    \dot{M}_b      &=& \frac{1}{2} M \veedot \Omega_b
    \label{eq:ode_system_body_frame}
\end{subeqnarray}
that can be solved as an initial value problem. \\
...
the system in eqn.~(\ref{eq:ode_system_body_frame}). \\
...
eqn.~(\ref{eq:ode_system_body_frame}). \\
